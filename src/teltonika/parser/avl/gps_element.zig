...
pub fn parseGpsElement(packet_data: []u8, cursor: *usize, allocator: std.mem.Allocator) !GpsElement {
    ...
    const longitude_f32 = @as(f32, @floatFromInt(longitude_raw)) / 10_000_000.0;
    const longitude_value = @as(f64, longitude_f32);
    gps_element.longitude = try std.fmt.allocPrint(allocator, "{d:.14}", .{longitude_value});

    const latitude_buf: [4]u8 = packet_data[start + 4 .. start + 8][0..4].*;
    const latitude_raw = std.mem.readInt(i32, &latitude_buf, .big);
    const latitude_f32 = @as(f32, @floatFromInt(latitude_raw)) / 10_000_000.0;
    const latitude_value = @as(f64, latitude_f32);
    gps_element.latitude = try std.fmt.allocPrint(allocator, "{d:.14}", .{latitude_value});
